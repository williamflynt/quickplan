...
const JS_TO_WEEKDAY: Weekday[] = ['su', 'm', 't', 'w', 'th', 'f', 's']

/** Parse a weekday string to our Weekday type. Case-insensitive. */
export function parseWeekday(s: string): Weekday {
  const lower = s.toLowerCase().trim()
  if (lower in WEEKDAY_TO_JS) return lower as Weekday
  throw new Error(
    `Invalid weekday: "${s}". Expected one of: m, t, w, th, f, s, su`,
  )
}

/** Parse a comma-separated workdays string like "m,t,w,th,f". */
export function parseWorkdays(s: string): Weekday[] {
  return s.split(',').map((d) => parseWeekday(d.trim()))
}

/** Get the JS Date.getDay() index for a weekday. */
export function weekdayIndex(d: Weekday): number {
 ...
 */
export function workdaySet(workdays: Weekday[]): Set<number> {
...
function isDayWorking(
  ...
  if (pto && pto.has(iso)) return false
  return workdays.has(date.getDay())
}
