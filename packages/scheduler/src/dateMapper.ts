...
export function offsetToDate(
  ...
  const workdays = workdaySet(
    resourceCalendar?.workdays ?? config.defaultWorkdays ?? DEFAULT_WORKDAYS,
  )
 ...
 */
export function dateToOffset(date: string, config: CalendarConfig): number {
  const start = fromIsoDate(config.startDate)
