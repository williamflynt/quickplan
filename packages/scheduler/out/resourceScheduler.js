...
import { fromIsoDate, toIsoDate, workdaySet, addWorkingDays, subtractWorkingDays, skipToWorkingDay, } from './workday.js';
/**
 ...
 */
export function scheduleResources(cpmResults, assignments, config, resourceCalendars) {
    const calendarMap = new Map(resourceCalendars.map((rc) => [rc.resourceName, rc]));
    ...
    for (const [resourceName, taskIds] of tasksByResource) {
        ...
        const resourceWorkdays = rc?.workdays ?? config.defaultWorkdays ?? DEFAULT_WORKDAYS;
        const workdays = workdaySet(resourceWorkdays);
        const holidays = new Set(config.holidays);
        const pto = rc ? new Set(rc.pto) : new Set();
        ...
        const rawTasks = taskIds
            ...
            .filter((t) => t.cpm !== undefined)
            .sort((a, b) => a.cpm.earliestStart - b.cpm.earliestStart);
        const tasks = [];
        let totalWorkingDays = 0;
        let resourceAvailableDate = skipToWorkingDay(projectStart, workdays, holidays, pto);
        for (const { taskId, cpm } of rawTasks) {
            let startIso;
            let finishIso;
            if (cpm.done && cpm.doneDate) {
                ...
                if (roundedDuration > 0) {
                    ...
                    const start = subtractWorkingDays(doneDateObj, roundedDuration, workdays, holidays, pto);
                ...
                resourceAvailableDate = skipToWorkingDay(nextDay, workdays, holidays, pto);
            }
            else {
                // Normal scheduling: map CPM earliest start to calendar date
                const cpmStartDate = addWorkingDays(new Date(projectStart), cpm.earliestStart, workdays, holidays, pto);
                // Task starts at the later of CPM start or resource availability
                const actualStartDate = cpmStartDate >= resourceAvailableDate
                    ? cpmStartDate
                    : resourceAvailableDate;
                // Skip to next working day in case we landed on a non-working day
                const startDate = skipToWorkingDay(actualStartDate, workdays, holidays, pto);
                ...
                const roundedDuration = Math.round(cpm.duration);
                const finishDate = roundedDuration > 0
                    ? addWorkingDays(new Date(startDate), roundedDuration, workdays, holidays, pto)
                    : new Date(startDate);
        ...
        if (tasks.length > 0) {
            ...
            calendarSpan = Math.round((lastFinish.getTime() - firstStart.getTime()) / (1000 * 60 * 60 * 24));
        }
        rows.push({
            resourceName,
            tasks,
...
//# sourceMappingURL=resourceScheduler.js.map