...
export const extractEntities = (
  ...
  if (isProjectConfig(node)) {
    ...
    if (typeof attrs.startDate === 'string') {
      p.calendarConfig = {
        ...
        workdays:
          typeof attrs.workdays === 'string' ? attrs.workdays : undefined,
        holidays:
          typeof attrs.holidays === 'string' ? attrs.holidays : undefined,
      }
    }
    return p
  }
  ...
  if (isCluster(node)) {
    ...
    for (const item of node.items) {
      p.clusters[node.name].items.add({
        name: item.name,
        type: item.$type,
        attributes: mapAttrs(item.attributes),
      })
      clock++
    }
    return p
...
abstract class RelationalIndex<S extends BaseEntity, T extends BaseEntity> {
  ...
  toJSON() {
    return Array.from(this.sourceIndex.entries()).reduce<Relation[]>(
      (acc, entry) => {
        return [
          ...acc,
          ...Array.from(entry[1]).map((item) => {
            return {
              source: { type: entry[0].type, name: entry[0].name },
              target: { type: item.type, name: item.name },
            }
          }),
        ]
      },
...
class DependencyIndex extends RelationalIndex<
  Task | Milestone,
  Task | Milestone
> {
...
const removeEntity = (
  node: AstTask | AstMilestone | AstResource | AstCluster,
  p: ProjectEntities,
): ProjectEntities => {
 ...
 */
const getOrCreate = (
  node: AstTask | AstMilestone | AstResource,
  p: ProjectEntities,
): Task | Milestone | Resource => {
  if (node.$type === 'Resource') {
    if (!p.resources[node.name]) {
      p.resources[node.name] = {
        name: node.name,
        type: 'Resource',
        attributes: mapAttrs(node.attributes),
      }
    } else {
      p.resources[node.name] = mergeAttrs(
        p.resources[node.name],
        node.attributes,
      )
    }
  ...
  if (node.$type === 'Milestone') {
    if (!p.milestones[node.name]) {
      p.milestones[node.name] = {
        name: node.name,
        type: 'Milestone',
        attributes: mapAttrs(node.attributes),
      }
    } else {
      p.milestones[node.name] = mergeAttrs(
        p.milestones[node.name],
        node.attributes,
      )
    }
