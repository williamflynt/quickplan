...
export const generateAction = async (
  ...
  const services =
    createProjectFlowSyntaxServices(NodeFileSystem).ProjectFlowSyntax
...
export const snapshotAction = async (
  fileName: string,
  opts: GenerateOptions,
): Promise<void> => {
  const services =
...
export default function (): void {
  ...
  const fileExtensions =
    ProjectFlowSyntaxLanguageMetaData.fileExtensions.join(', ')
  program
    .command('generate')
    .argument(
      '<file>',
    ...
    .description(
      'generates an AST representation of the source file in JSON format and prints to console',
    )
  ...
  program
    ...
    .description(
      'parses the source file and generates a snapshot of the project and entities',
    )
