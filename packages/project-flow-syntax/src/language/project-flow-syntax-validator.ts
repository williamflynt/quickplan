...
import type {ProjectFlowSyntaxAstType, DependencyChain} from './generated/ast.js';
...
export class ProjectFlowSyntaxValidator {
        ...
        for (const segment of chain.segments) {
            ...
            if (segment.remove) {
                for (const left of leftMost) {
                    ...
                    for (const right of segment.rightSide) {
                        const rightKey = GraphNode.nameFor(right);
                        this.dependencyNodes[leftKey].removeOutgoing(this.dependencyNodes[rightKey]);
                    }
                }
            ...
            for (let i = 0; i < segment.rightSide.length; i++) {
                ...
                for (const left of leftMost) {
                    ...
                    // ADD
                    this.dependencyNodes[leftKey].addOutgoing(this.dependencyNodes[rightKey]);
                    ...
                    if (sourceCycle.length > 0) {
                        ...
                        accept('error', `Cycle detected in dependency chain: ${cycleStr}`,
                            {node: segment, property: 'rightSide', index: i});
                    }
                }
            }
            leftMost = segment.rightSide;
    ...
    private detectCycleFromSources(): string[] {
        const sourceNodes = Object.values(this.dependencyNodes).filter(node => node.inDegree === 0);

        // If there are no source nodes, but we have nodes, we have a cycle.
        if (sourceNodes.length === 0 && Object.keys(this.dependencyNodes).length > 0) {
...
class GraphNode {
    ...
    static nameFor(node: NamedTypedNode): string {
        const sigil = node.$type === 'Task' ? '' : node.$type === 'Milestone' ? '%' : 'ERROR';
        if (sigil === 'ERROR') {
