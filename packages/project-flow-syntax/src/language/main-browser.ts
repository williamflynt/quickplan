...
const { shared, ProjectFlowSyntax } = createProjectFlowSyntaxServices({ connection, ...EmptyFileSystem });
...
shared.workspace.DocumentBuilder.onBuildPhase(DocumentState.Validated, documents => {
    for (const document of documents) {
        ...
        void connection.sendNotification(documentChangeNotification, notificationContent);
    }
});
