...
describe('CPM Critical Path Calculation', () => {
    test('should identify critical path edges correctly', async () => {
        ...
        const writeTestsEdge = result.edges.find((e) => e.id === 'Task:WriteTests > Task:Test');
    ...
    test('should generate edge IDs that match the format used by edgeBuilder', async () => {
        ...
        if ('error' in result)
            return;
        const edge = result.edges.find((e) => e.from === 'Task:A' && e.to === 'Task:B');
...
//# sourceMappingURL=cpm.test.js.map