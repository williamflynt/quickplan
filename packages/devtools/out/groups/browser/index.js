...
function cdpPort(rawArgs) {
    return parseInt(rawArgs.cdpPort ?? '9222', 10);
}
...
// ── content ─────────────────────────────────────────────────────────
async function executeContent(_ctx, rawArgs) {
    const format = rawArgs.format ?? 'text';
...
// ── navigate ────────────────────────────────────────────────────────
async function executeNavigate(_ctx, rawArgs) {
    const url = rawArgs.url;
    const waitUntil = rawArgs.waitUntil ?? 'load';
    return withClient(cdpPort(rawArgs), async (client) => {
...
// ── eval ────────────────────────────────────────────────────────────
async function executeEval(_ctx, rawArgs) {
...
// ── click ───────────────────────────────────────────────────────────
async function executeClick(_ctx, rawArgs) {
    const selector = rawArgs.selector;
    return withClient(cdpPort(rawArgs), async (client) => {
...
// ── type ────────────────────────────────────────────────────────────
async function executeType(_ctx, rawArgs) {
    const selector = rawArgs.selector;
    const text = rawArgs.text;
    const delay = parseInt(rawArgs.delay ?? '0', 10);
    return withClient(cdpPort(rawArgs), async (client) => {
...
// ── wait ────────────────────────────────────────────────────────────
async function executeWait(_ctx, rawArgs) {
    const selector = rawArgs.selector;
    const timeout = parseInt(rawArgs.timeout ?? '5000', 10);
    return withClient(cdpPort(rawArgs), async (client) => {
...
// ── screenshot ──────────────────────────────────────────────────────
async function executeScreenshot(_ctx, rawArgs) {
    const output = rawArgs.output;
    const fullPage = rawArgs.fullPage;
    return withClient(cdpPort(rawArgs), async (client) => {
...
// ── set-editor ─────────────────────────────────────────────────────
async function executeSetEditor(_ctx, rawArgs) {
    const text = rawArgs.text;
...
async function executeLogs(ctx, rawArgs) {
    ...
    const lines = parseInt(rawArgs.lines ?? '50', 10);
    ...
    try {
        ...
        const duration = parseInt(rawArgs.duration ?? '2000', 10);
...
// ── targets ─────────────────────────────────────────────────────────
async function executeTargets(_ctx, rawArgs) {
    const port = cdpPort(rawArgs);
    const client = new CDPClient();
    try {
