...
async function executeTest(
  ...
  try {
    ...
    const tools =
      ((listResp.result as Record<string, unknown>)?.tools as {
        name: string
      }[]) ?? []
...
async function executeCall(
  ...
  if (args.args) {
    try {
      ...
      return {
        ok: false,
        error: `Invalid JSON for tool arguments: ${args.args}`,
      }
    }
  }
...
async function executeTools(ctx: CommandContext): Promise<CommandResult> {
...
async function executeMcpLogs(
  ...
  if (!existsSync(logPath)) {
    return {
      ok: false,
      error: 'No MCP server log found. Has the server been started?',
    }
  }
  ...
  commands: [
    {
      name: 'test',
      description:
        'Smoke-test the MCP server: initialize, list tools, and make a sample call',
      execute: executeTest,
    },
    {
      name: 'call',
