...
export const NodeForm: FC<NodeFormProps> = ({ node }) => {
  ...
  const onFinish = (values: Record<string, unknown>) => {
    ...
    api
      .graphActivityPatch(activeChartId, node.id, asStr)
      .then((response) => {
  ...
  return (
    ...
    >
      ...
      >
        <InputNumber
          style={{ width: '100%' }}
          placeholder={node.data.cpm.durationLow.toString()}
        />
      </Form.Item>
      <Form.Item
        name={'durationLikely'}
        label={'DurationLikely'}
        initialValue={undefined}
      >
        <InputNumber
          style={{ width: '100%' }}
          placeholder={node.data.cpm.durationLikely.toString()}
        />
      </Form.Item>
      <Form.Item
        name="durationHigh"
      ...
      >
        <InputNumber
          style={{ width: '100%' }}
          placeholder={node.data.cpm.durationHigh.toString()}
        />
      </Form.Item>
